...
public class RCASchedulerTaskTests {
    ...
    /**
     * Given a dependency graph, this test checks that the intents are sent to the right nodes.
     * And after the receipt of the intent, the data is sent to the nodes that have expressed the intent.
     */
    @Test
    public void getLocallyExecutableNodes() {
        ...
        AnalysisGraph graph = new AnalysisGraph() {
            ...
            */
            ...
            public void construct() {
                ...
                Symptom moonSymptom = new Symptom(1) {
                    @Override
                    public SymptomFlowUnit operate() {
                        return SymptomFlowUnit.generic();
                    }
                ...
                Symptom skyLabsSymptom = new Symptom(1) {
                    @Override
                    public SymptomFlowUnit operate() {
                        return SymptomFlowUnit.generic();
                    }
                ...
                skyLabsSymptom.addAllUpstreams(new ArrayList<Node>() {{
                    add(earthSymptom);
                    add(moonSymptom);
                }});
        ...
        RcaConf skyLabsConf = new RcaConf() {
            @Override
            public Map<String, String> getTagMap() {
                return new HashMap<String, String>() {{
        ...
        class WireHopperDerived extends WireHopper {
            ...
            public void sendIntent(IntentMsg intentMsg) {
                assertEquals(intentMsg.getRequesterNode(), this.intentMsgs.get(intextIdx).getRequesterNode());
                assertEquals(intentMsg.getDestinationNode(), this.intentMsgs.get(intextIdx).getDestinationNode());
                intextIdx++;
            }
        };
        ...
        class RCASchedulerTaskMock extends RCASchedulerTask {
            private RCASchedulerTaskMock(RcaConf conf, WireHopper wireHopper) {
                super(100, Executors.newFixedThreadPool(2), connectedComponents, null,
                        null, conf, wireHopper);
            }
        }

        RCASchedulerTaskMock earthTask = new RCASchedulerTaskMock(earthConf,
                new WireHopperDerived(Collections.emptyList(),
                    new DataMsg(EARTH_KEY, new ArrayList<String>() {{
                        add(MOON_KEY);
                        add(SKY_LABS_KEY);
                    }},
                            null)));

        RCASchedulerTaskMock moonTask = new RCASchedulerTaskMock(moonConf,
                new WireHopperDerived(new IntentMsg(MOON_KEY, EARTH_KEY, null),
                        new DataMsg(MOON_KEY, new ArrayList<String>() {{ add(SKY_LABS_KEY);}}, null)));
        RCASchedulerTaskMock skyLabsTask = new RCASchedulerTaskMock(skyLabsConf,
