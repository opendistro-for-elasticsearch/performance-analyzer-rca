...
public class HighCpuSymptomTest {
    ...
    static class ShardCpuHighSymptom extends Symptom {
        ...
        public <M extends Metric> ShardCpuHighSymptom(long evaluationIntervalSeconds, M cpu_UtilizationX) {
            super(evaluationIntervalSeconds);
            this.cpu_UtilizationX = cpu_UtilizationX;
        }

        // The operate here tries to find hot shard. The way it tries to do it is creates a MovingAverage object for
        // each distinct shardID. The moving average window is defined as three samples for less that three it
        // outputs -1 and for greater than or equal to 3, it outputs the average for that window. This method
        // compares it against an arbitrary threshold of 90% and if such a shard exists, then it reports it as a flow
        // unit. So the symptom can be defined as if there exists a shard for which the average of max is greater
        // than 90% for three consecutive samples, then classify it as hot. This is not accurate in production but it
        ...
        public SymptomFlowUnit operate() {
            ...
            for (int i=1; i<allData.size(); i++) {
                ...
                if (val > HIGH_CPU_THRESHOLD) {
                    ...
                    ret.add(dataRow);
                    System.out.println(String.format("Shard %s is hot. Average max CPU (%f) above: %f", shardId,
                            val, HIGH_CPU_THRESHOLD));
                    shouldReportOperation = true;
                }
            }

            if (shouldReportOperation) {
                return new SymptomFlowUnit(System.currentTimeMillis(), ret,
                        new SymptomContext(SymptomContext.State.UNHEALTHY));
            }
            else {
                return new SymptomFlowUnit(System.currentTimeMillis(),
                        new SymptomContext(SymptomContext.State.HEALTHY));
            }
        }
    }
    ...
    public void testSymptomCreation() throws Exception {
        ...
        ((MetricsDBProviderTestHelper)queryable).addNewData(CPU_Utilization.NAME,
                Arrays.asList("shard1", "index3", "bulk", "primary"), 92.4);
        ((MetricsDBProviderTestHelper)queryable).addNewData(CPU_Utilization.NAME,
                Arrays.asList("shard2", "index3", "bulk", "primary"), 93.4);
        ((MetricsDBProviderTestHelper)queryable).addNewData(CPU_Utilization.NAME,
                Arrays.asList("shard1", "index3", "bulk", "primary"), 95.4);
        ((MetricsDBProviderTestHelper)queryable).addNewData(CPU_Utilization.NAME,
                Arrays.asList("shard2", "index3", "bulk", "primary"), 4.4);
        ((MetricsDBProviderTestHelper)queryable).addNewData(CPU_Utilization.NAME,
                Arrays.asList("shard1", "index3", "bulk", "primary"), 90.4);
        ((MetricsDBProviderTestHelper)queryable).addNewData(CPU_Utilization.NAME,
                Arrays.asList("shard2", "index3", "bulk", "primary"), 5.4);
            ...
            for (List<Node> nodeList: component.getAllNodesByDependencyOrder()) {
                for (Node node: nodeList) {
                    if (node instanceof Metric) {
                        List<MetricFlowUnit> flowUnits = Collections.singletonList(((Metric)node).gather(queryable));
