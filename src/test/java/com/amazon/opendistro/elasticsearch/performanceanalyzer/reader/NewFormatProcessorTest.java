...
public class NewFormatProcessorTest {
  ...
  public void processNewMetricsFormat() throws Exception {
    ...
    Path configPath =
        Paths.get(path.toString(), "test_files/new_format/performance-analyzer.properties");
    System.out.println("==" + configPath + "==");
    System.setProperty("configFilePath", configPath.toString());

    ReaderMetricsProcessor readerMetricsProcessor =
        new ReaderMetricsProcessor(DUMP_DIR + "/new_format", true);

    EventLogFileHandler eventLogFileHandler =
        new EventLogFileHandler(new EventLog(), DUMP_DIR + "/new_format");
  ...
  public void processOldMetricsFormat() throws Exception {
    Path path = Paths.get(System.getProperty("user.dir"));
    Path configPath =
        Paths.get(path.toString(), "test_files/old_format/performance-analyzer.properties");
    System.out.println("==" + configPath + "==");
    System.setProperty("configFilePath", configPath.toString());

    ReaderMetricsProcessor readerMetricsProcessor =
        new ReaderMetricsProcessor(DUMP_DIR + "/old_format");
    readerMetricsProcessor.processMetrics(DUMP_DIR + "/old_format", 1566413970000L);
  }

  private ReaderMetricsProcessorDummy createReaderMetricsProcessor(DATA_FORMAT format)
      throws Exception {
    Path path = Paths.get(System.getProperty("user.dir"));
    Path configPath =
        Paths.get(
            path.toString(), "test_files/" + format.format + "/performance-analyzer.properties");
    System.out.println("==" + configPath + "==");
    System.setProperty("configFilePath", configPath.toString());
  ...
  // The idea is to call processNodeMetrics with the old format and the new format and compare the
  // results.
  @Test
  ...
  private void compareOsMetricsMaps(
      NavigableMap<Long, OSMetricsSnapshot> old, NavigableMap<Long, OSMetricsSnapshot> nw) {
  ...
  private void compareReqMetricsMaps(
      NavigableMap<Long, ShardRequestMetricsSnapshot> old,
  ...
  private void compareHttpReqMetricsMaps(
      NavigableMap<Long, HttpRequestMetricsSnapshot> old,
  ...
  private void compareMasterReqMetricsMaps(
      NavigableMap<Long, MasterEventMetricsSnapshot> old,
  ...
  private void compareNodeMetricsMaps(
      Map<AllMetrics.MetricName, NavigableMap<Long, MemoryDBSnapshot>> old,
      Map<AllMetrics.MetricName, NavigableMap<Long, MemoryDBSnapshot>> nw) {
    Assert.assertEquals(old.size(), nw.size());

    for (Map.Entry<AllMetrics.MetricName, NavigableMap<Long, MemoryDBSnapshot>> entryOld :
        old.entrySet()) {
  ...
  static class ReaderMetricsProcessorDummy extends ReaderMetricsProcessor {
    ...
    ReaderMetricsProcessorDummy(DATA_FORMAT format) throws Exception {
      super(DUMP_DIR + "/" + format.format, format == DATA_FORMAT.NEW);
    ...
    public NodeMetricsEventProcessor parseNodeMetrics(long timestamp) throws Exception {
      switch (format) {
        ...
        case NEW:
          ...
          EventLogFileHandler eventLogFileHandler =
              new EventLogFileHandler(new EventLog(), rootLocation);
