...
public class ReaderTrimDBTests {
  ...
  /**
   * The trimDatabase removes all the in-memory entries from the map provided to it as an argument,
   * leaving behind the maxFiles number of entries. This essentially tests that. Deleting the
   * in-memory entries may or may not clean up the files from the disk. This is controlled by the
   * the third argument. If set to false, the the files on disk are left behind. So essentially, no
  ...
  public void testMetricsDBFilesNotCleaned() throws Exception {
    ...
    // Goal is to clean up the in-memory entries leaving behind the maxFiles most recent ones, but
    // to leave the
  ...
  /**
   * A counterpart of the testMetricsDBFilesNotCleaned test but checks that the files are cleaned up
   * when the deleteDBFiles boolean is set to true.
   *
   * @throws Exception if creating metricsDB file fails
   */
  @Test
  public void testMetricsDBFilesCleaned() throws Exception {
    ...
    // The idea is to clean up the in memory entries and the on disk files leaving behind only the
    // maxFiles
    // number of entries.
