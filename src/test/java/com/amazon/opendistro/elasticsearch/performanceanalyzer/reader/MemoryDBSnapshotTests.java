...
public class MemoryDBSnapshotTests extends AbstractReaderTests {
  ...
  public MemoryDBSnapshotTests() throws SQLException, ClassNotFoundException {
    ...
    fields[0] = DSL.field(CircuitBreakerValue.CB_ESTIMATED_SIZE.toString(), Double.class);
    fields[1] = DSL.field(CircuitBreakerValue.CB_CONFIGURED_SIZE.toString(), Double.class);
    fields[2] = DSL.field(CircuitBreakerValue.CB_TRIPPED_EVENTS.toString(), Double.class);
  }
  ...
  public void testCreateMemoryDBSnapshot() throws Exception {
    ...
    MemoryDBSnapshot circuitBreakerValuesSnap =
        new MemoryDBSnapshot(conn, MetricName.CIRCUIT_BREAKER, windowEndTime);

    assertEquals(
        MetricName.CIRCUIT_BREAKER.toString() + windowEndTime,
    ...
    Result<Record> resultRecord =
        circuitBreakerValuesSnap.fetchMetric(
            getDimensionEqCondition(
                CircuitBreakerDimension.CB_TYPE, CircuitBreakerType.request.toString()),
            fields);
  ...
  public void testAlignWindow() throws Exception {
    ...
    MemoryDBSnapshot circuitBreakerValuesSnap1 =
        new MemoryDBSnapshot(conn, MetricName.CIRCUIT_BREAKER, 6000L);
    ...
    MemoryDBSnapshot circuitBreakerValuesSnap2 =
        new MemoryDBSnapshot(conn, MetricName.CIRCUIT_BREAKER, 11000L);
    ...
    MemoryDBSnapshot circuitFinal =
        new MemoryDBSnapshot(conn, MetricName.CIRCUIT_BREAKER, 10000L, true);
