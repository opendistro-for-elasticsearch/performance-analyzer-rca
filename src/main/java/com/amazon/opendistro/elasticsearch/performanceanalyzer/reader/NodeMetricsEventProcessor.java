...
public class NodeMetricsEventProcessor implements EventProcessor {
    ...
    private NodeMetricsEventProcessor(Map<AllMetrics.MetricName,
            MemoryDBSnapshot> metricsSnapshotMap) {
        this.metricsSnapshotMap = metricsSnapshotMap;
        this.metricsBatchBindMap = new HashMap<>();
    }

    static NodeMetricsEventProcessor buildNodeMetricEventsProcessor(long currTimestamp,
                                                                    Connection conn,
                                                                    Map<AllMetrics.MetricName,
                                                              NavigableMap<Long, MemoryDBSnapshot>> nodeMetricsMap)
            throws Exception {
        Map<AllMetrics.MetricName, MemoryDBSnapshot> metricSnapshotMap =
                new HashMap<>();

        for (AllMetrics.MetricName metric : AllMetrics.MetricName.values()) {
            NavigableMap<Long, MemoryDBSnapshot> currMetricMap =
                    nodeMetricsMap.get(metric);
        ...
        NodeMetricsEventProcessor eventProcessor =
                new NodeMetricsEventProcessor(metricSnapshotMap);
    ...
    public void finalizeProcessing() {
        for (AllMetrics.MetricName metric : AllMetrics.MetricName.values()) {
            ...
            if (batchHandle != null && batchHandle.size() > 0) {
                batchHandle.execute();
                NavigableMap<Long, MemoryDBSnapshot> currMap =
                        nodeMetricsMap.get(metric);
    ...
    public void processEvent(Event nodeMetric) {
        String key = nodeMetric.key.split(File.separatorChar == '\\' ?
                "\\\\" : File.separator)[0];
        AllMetrics.MetricName name = MetricPropertiesConfig
                .getInstance()
                .getEventKeyToMetricNameMap()
                .get(key);

        MemoryDBSnapshot snap = metricsSnapshotMap.get(name);
        BatchBindStep batchHandler = metricsBatchBindMap.computeIfAbsent(name,
                k -> snap.startBatchPut());
        MetricProperties currParser =
                MetricPropertiesConfig.getInstance().getProperty(name);
    ...
    public boolean shouldProcessEvent(Event event) {
        for (String metric : MetricPropertiesConfig
                    .getInstance().getMetricPathMap().values()) {
    ...
    public void commitBatchIfRequired() {
        if (lastUpdatedMetric != null) {
            BatchBindStep handle =
                    metricsBatchBindMap.get(lastUpdatedMetric);
            if (handle.size() > BATCH_LIMIT) {
                handle.execute();
                metricsBatchBindMap.put(lastUpdatedMetric,
                        metricsSnapshotMap.get(lastUpdatedMetric).startBatchPut());
            }
        }
    }

    private boolean processEvent(Event event, MemoryDBSnapshot snap,
                                     long startTime, BatchBindStep batchHandle,
        ...
        String[] derivedDimension =
                metricProperties.getHandler().processExtraDimensions(event.key);
        ...
        for (int lineNum = 1; lineNum < lines.length; lineNum++) {
            processed = metricProperties.processJsonLine(lines[lineNum],
                    batchHandle,
