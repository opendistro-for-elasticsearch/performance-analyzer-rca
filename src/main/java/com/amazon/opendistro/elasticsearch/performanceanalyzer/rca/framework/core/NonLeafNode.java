...
public abstract class NonLeafNode extends Node  implements Operable {
    ...
    /**
     * Add all the upstream nodes of this node in one shot. This method cannot be called on a same node twice.
     * This restriction is important to ensure the absence of cycles.
    ...
    /**
     * Adding the upstream nodes to the current node requires some validations.
     * 1. The call to the allAllUpstreams should be made only once.
     * 2. The evaluation interval of the node is greater than or equal to all the nodes it depends on.
     * 3. The Metrics this node depends on should already be added to the FlowField.
     *
     */
    private int validateAndAddDownstream(List<Node> upstreams) {
        ...
        for (Node node: upstreams) {
            ...
            // When upstreams are added to a node, this also leads to the possibility of merging two graphs if they
            // are not already. If so, we want to use the min of the two IDs as the ID of the merged graph.
        ...
        if (foundNotAddedMetrics) {
            throw new MalformedAnalysisGraph(String.format("These metrics are not added to the AnalysisGraph yet: %s ",
