    ...
    public ResourceFlowUnit operate() {
        ...
        if (counter == rcaPeriod) {
            ...
            for (IndexShardKey indexShardKey : indexShardKeySet) {
                double avgCpuUsage = cpuUsageHashMap.get(indexShardKey).readAvg(TimeUnit.SECONDS);
                double avgIoTotThroughput = ioTotThroughputHashMap.get(indexShardKey).readAvg(TimeUnit.SECONDS);
                double avgIoTotSyscallRate = ioTotSyscallRateHashMap.get(indexShardKey).readAvg(TimeUnit.SECONDS);
            ...
            // reset the variables
            counter = 0;
            cpuUsageHashMap.clear();
            ioTotThroughputHashMap.clear();
            ioTotSyscallRateHashMap.clear();
