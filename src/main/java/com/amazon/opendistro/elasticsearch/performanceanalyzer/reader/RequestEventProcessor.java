...
public class RequestEventProcessor implements EventProcessor {
    ...
    static RequestEventProcessor buildRequestMetricEventsProcessor(long currWindowStartTime,
                                                                   long currWindowEndTime,
                                                                   Connection conn,
                                                                   NavigableMap<Long,
                                                             ShardRequestMetricsSnapshot> shardRqMetricsMap
    ) throws Exception {
        if (shardRqMetricsMap.get(currWindowStartTime) == null) {
            ShardRequestMetricsSnapshot rqSnap =
                    new ShardRequestMetricsSnapshot(conn,
                            currWindowStartTime);
            Map.Entry<Long, ShardRequestMetricsSnapshot> entry =
                    shardRqMetricsMap.lastEntry();
            if (entry != null) {
    ...
    private void emitStartMetric(String[] metricKeyPathElements, Map<String, String> keyValueMap) {
        ...
        long docCount = Long.parseLong(keyValueMap.computeIfAbsent(ShardBulkMetric.ITEM_COUNT.toString(), k -> "0"));
