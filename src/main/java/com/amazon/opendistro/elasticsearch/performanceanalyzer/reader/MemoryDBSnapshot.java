...
public class MemoryDBSnapshot implements Removable {
    private static final Logger LOG = LogManager
            .getLogger(MemoryDBSnapshot.class);
   ...
   /**
     ...
     *            (Let's call it readerProcessTime). Reader process
     *            /dev/shm/performanceanalyzer files every 5 seconds. When creating aligned db
     *            snapshot, we use previous window's end time. Note the previous
     *            window's end time is computed after using
     *            PerformanceAnalyzerMetrics.getTimeInterval(). So if previous window's actual
    ...
    public MemoryDBSnapshot(Connection conn, MetricName tableNamePrefix,
        ...
        dimensionNames = MetricPropertiesConfig.getInstance()
                ...
                .getDimensionNames();
        dimensionsFields = MetricPropertiesConfig.getInstance()
                .getProperty(tableNamePrefix)
                .getDimensionFields();
        metadata = MetricPropertiesConfig.getInstance()
                .getProperty(tableNamePrefix)
                .getMetricFields();

        tableSelectFieldsMap = MetricPropertiesConfig.getInstance()
                .getProperty(tableNamePrefix).getTableSelectMap();
        tableGroupByFieldsMap =  MetricPropertiesConfig.getInstance()
                .getProperty(tableNamePrefix).getTableGroupByFieldsMap();
        tableWhereClauseMap = MetricPropertiesConfig.getInstance()
                .getProperty(tableNamePrefix).getTableWhereClauseMap();
    ...
    /**
     *
     * @return the last update time of the /dev/shm/performanceanalyzer file that is the data
     *         source of our db table.
     */
    ...
    public BatchBindStep startBatchPut() {
        ...
        return create.batch(create.insertInto(DSL.table(this.tableName))
                .values(dummyValues));
    }
     ...
     *            |-----------alignedWindow------|
     *
     * This method assumes that both left/right windows are greater than or
     * equal to 5 seconds.
     *
     * @param leftWindow
     *            MemoryDBSnapshot for the /dev/shm/performanceanalyzer file written before t.
     *            We save MemoryDBSnapshot in a map where the key is the time at
     *            which the /dev/shm/performanceanalyzer file was written, and the value is the
     *            MemoryDBSnapshot itself.
     * @param rightWindow
     *            MemoryDBSnapshot for the /dev/shm/performanceanalyzer file written after t
     * @param a
     *            aligned window start time.
     * @param b
     *            aligned window end time.
     * @param t
     *            leftWindow end time, as well as right window start time
     */
    public void alignWindow(MemoryDBSnapshot leftWindow,
            MemoryDBSnapshot rightWindow, long t, long a,
            long b) {
        ArrayList<SelectField<?>> alignedFields =
        ...
        SelectJoinStep<Record> recordsSource = create
        .select(alignedFields)
        .from(create.select(leftWinFields).from(leftWindow.tableName)
                .unionAll(create.select(rightWinFields)
                        .from(rightWindow.getTableName())));

        if (getDimensions().isEmpty()) {
            create.insertInto(DSL.table(this.tableName)).select(recordsSource)
                    .execute();
        } else {
            create.insertInto(DSL.table(this.tableName))
                    .select(recordsSource.groupBy(getDimensions())).execute();
        }
    }

    /**
     * Precondition: The order of columns in each values[i] should match the
     *  table we have created in the constructor.  We cannot check this
     *   programmatically.  People who write code calling this method is
     *    responsible for verification.
     *
     * @param values
     *            each values[i] is a row
     */
    ...
    protected Result<Record> fetchMetric(
            Condition condition, SelectField<?>... column) {
        return create.select(column)
                .from(DSL.table(this.tableName)).where(condition).fetch();
    }
    ...
    public Map<String, SelectHavingStep<Record>> selectMetadataSource() {
        ...
        for (Map.Entry<String, List<Field<?>>> entry : tableSelectFieldsMap
                .entrySet()) {
            String tableName = entry.getKey();
            selectFromTable.put(tableName,
                    create.select(entry.getValue()).from(this.tableName)
