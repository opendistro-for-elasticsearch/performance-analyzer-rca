...
public class MasterMetricsEventProcessor implements EventProcessor {
    ...
    static MasterMetricsEventProcessor buildMasterMetricEventsProcessor(long currWindowStartTime,
                                                                        Connection conn,
                                                                        NavigableMap<Long,
                                                                       MasterEventMetricsSnapshot
                                                                       > masterEventMetricsMap) {
        MasterEventMetricsSnapshot masterSnap =
                masterEventMetricsMap.get(currWindowStartTime);
        if (masterSnap == null) {
            masterSnap = new MasterEventMetricsSnapshot(conn, currWindowStartTime);
            Map.Entry<Long, MasterEventMetricsSnapshot> entry =
                    masterEventMetricsMap.lastEntry();
            if (entry != null) {
    ...
    public void finalizeProcessing() {
        ...
        LOG.info("Final masterEvents request metrics {}",
                masterSnap.fetchAll());
    }

    @Override
    public void processEvent(Event event) {
        String[] keyElements = event.key.split(File.separatorChar == '\\' ?
                "\\\\" : File.separator);
    ...
    private void emitStartMasterEventMetric(Event entry,
                                            String insertOrder,
                                            String threadId) {

        Map<String, String> keyValueMap =
                MetricsParser.extractEntryData(entry.value);
        String priority = keyValueMap.get(
                AllMetrics.MasterMetricDimensions.MASTER_TASK_PRIORITY.toString());
        long st = Long.parseLong(keyValueMap.get(
                AllMetrics.CommonMetric.START_TIME.toString()));
        String taskType = keyValueMap.get(
                AllMetrics.MasterMetricDimensions.MASTER_TASK_TYPE.toString());
        String taskMetadata = keyValueMap.get(
                AllMetrics.MasterMetricDimensions.MASTER_TASK_METADATA.toString());
        long queueTime = Long.parseLong(keyValueMap.get(
    ...
    private void emitEndMasterEventMetric(Event entry,
                                          String insertOrder,
                                          String threadId) {
        Map<String, String> keyValueMap =
                MetricsParser.extractEntryData(entry.value);
        long finishTime = Long.parseLong(keyValueMap.get(
