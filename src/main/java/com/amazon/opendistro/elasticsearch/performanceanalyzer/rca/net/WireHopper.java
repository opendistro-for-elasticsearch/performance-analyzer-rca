...
public class WireHopper {
    ...
    public WireHopper(final NetPersistor persistor, final NodeStateManager nodeStateManager, final NetClient netClient, final SubscriptionManager subscriptionManager) {
    ...
    public void sendIntent(IntentMsg msg) {
        subscriptionManager.broadcastSubscribeRequest(msg.getRequesterNode(), msg.getDestinationNode(), msg.getRcaConfTags());
    }
        ...
        if (subscriptionManager.isNodeSubscribed(sourceNode)) {
            ...
            for (final String downstreamHostAddress : downstreamHostAddresses) {
                for (final GenericFlowUnit flowUnit : dataMsg.getFlowUnits()) {
                    netClient.publish(downstreamHostAddress, flowUnit.buildFlowUnitMessage(sourceNode, esNode), new StreamObserver<PublishResponse>() {
                        @Override
                        ...
                        public void onError(final Throwable t) {
                            ...
                            subscriptionManager.unsubscribe(sourceNode, downstreamHostAddress);
                            //TODO: When an error happens, onCompleted is not guaranteed. So, terminate the connection.
                        }
        ...
        if (remoteFlowUnits.size() < publisherSet.size()) {
            for (final String publisher : publisherSet) {
                ...
                if (System.currentTimeMillis() - lastRxTimestamp > 2 * intervalInSeconds * MS_IN_S) {
                    LOG.debug("{} hasn't published in a while.. nothing from the last {} intervals", publisher,
    ...
    private void resendIntent(final String node, final String remoteHost, final Map<String, String> tags) {
