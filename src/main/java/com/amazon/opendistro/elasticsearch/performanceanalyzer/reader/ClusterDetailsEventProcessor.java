...
public class ClusterDetailsEventProcessor implements EventProcessor {
    ...
    public void processEvent(Event event) {
        ...
        if (lines.length < 2) {
            ...
            LOG.error("ClusterDetails contain less items than expected. " +
                    "Expected 2, found: {}", event.value);
            return;
        }
        ...
        ClusterLevelMetricsReader.NodeDetails[] tmpNodesDetails = new ClusterLevelMetricsReader.NodeDetails[lines.length - 1];
        ...
        for (int i = 1; i < lines.length; ++i) {
            ClusterLevelMetricsReader.NodeDetails nodeDetails = new ClusterLevelMetricsReader.NodeDetails(lines[i]);
