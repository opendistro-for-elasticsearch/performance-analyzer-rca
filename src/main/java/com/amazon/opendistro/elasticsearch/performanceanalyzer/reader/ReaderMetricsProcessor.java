...
public class ReaderMetricsProcessor implements Runnable {
  ...
  /**
   * Parse per thread OS metrics. OS metrics are generated per thread and written to files in
   * /dev/shm/performanceanalyzer/{rotation_window}/threads/{tid}/os_metrics. This function parses
   * the files written since the last successful run and populates an inmemory sqlite table with the
   * results. A few metrics available are - cpu, rss, minor pagefaults etc.
   *
  ...
  /**
   * Parse per node metric. Node level metrics are generated per node This function parses metrics
   * files written since the last successful run and populates an in-memory sqlite table with the
   * results. A few metrics available are - Starting memory limit for overall parent-level breaker,
   * maximum limit defined for the survivor memory pool (-1 means there is no maxium limit) etc.
   *
   * @param currTimestamp when reader starts processing metric files
   * @throws Exception thrown if we have issues parsing metrics
   */
  public void parseNodeMetrics(long currTimestamp, boolean testOnly) throws Exception {
    long mCurrT = System.currentTimeMillis();
    ...
    for (Map.Entry<AllMetrics.MetricName, NavigableMap<Long, MemoryDBSnapshot>> entry :
      ...
      MemoryDBSnapshot currSnap = new MemoryDBSnapshot(this.conn, name, currTimestamp);

      // parse metrics for last window.
      Map.Entry<Long, MemoryDBSnapshot> lastNodeMetricsEntry = currMap.lastEntry();
      ...
      if (currParser.dispatch(currSnap, currTimestamp, lastNodeMetricsSnapshotTime)) {
        LOG.info(
            "Adding new {} snapshot- currTimestamp {}, actualTime {}",
            name,
            currTimestamp,
            currSnap.getLastUpdatedTime());
        currMap.put(currSnap.getLastUpdatedTime(), currSnap);
      } else {
        LOG.info("Did not add values into {} snapshot. Clearing it {}", name, currTimestamp);
        currSnap.remove();
      }

      long mFinalT = System.currentTimeMillis();
      LOG.info("Total time taken for parsing {} Metrics: {}", name, mFinalT - mCurrT);
    }
  }

  /**
   * Parse all events generated by shard requests. This function processes shard events such as
   * shardBulk and shardSearch. Every operation emits a start event and an end event. The events can
   * be found in /dev/shm/performanceanalyzer/{rotation_window}/threads/{tid}/{operation}/{rid}/.
   * The start event data is written to a file called start and the end event data is in a file
   * called end.
   *
   * @param rootLocation where to find metric files
   ...
   * @param currWindowEndTime the end time of current sampling period. The bound of the period where
   *     that value is measured is MetricsConfiguration.SAMPLING_INTERVAL.
   * @throws Exception thrown if we have issues parsing metrics
   */
  protected void parseRequestMetrics(
      String rootLocation, long currWindowStartTime, long currWindowEndTime) throws Exception {
    long mCurrT = System.currentTimeMillis();
    if (shardRqMetricsMap.get(currWindowStartTime) == null) {
      ShardRequestMetricsSnapshot rqSnap =
          new ShardRequestMetricsSnapshot(this.conn, currWindowStartTime);
      ...
      metricsParser.parseRequestMetrics(
          rootLocation, currWindowStartTime, currWindowEndTime, rqSnap);
  ...
  /**
   * Parse all http events generated by shard requests. This function processes http events such as
   * bulk and search. Every operation emits a start event and an end event. The events can be found
   * in /dev/shm/performanceanalyzer/{rotation_window}/threads/{tid}/http/{operation}/{rid}/. The
   * start event data is written to * a file called start and the end event data is in a file called
   * end.
   *
   * @param rootLocation where to find metric files
   * @param currWindowStartTime the start time of current sampling period. The bound of the period
   ...
   * @throws Exception thrown if we have issues parsing metrics
   */
  protected void parseHttpRequestMetrics(
      String rootLocation, long currWindowStartTime, long currWindowEndTime) throws Exception {
    long mCurrT = System.currentTimeMillis();
    if (httpRqMetricsMap.get(currWindowStartTime) == null) {
      HttpRequestMetricsSnapshot httpRqSnap =
          new HttpRequestMetricsSnapshot(this.conn, currWindowStartTime);
      ...
      metricsParser.parseHttpMetrics(
          rootLocation, currWindowStartTime, currWindowEndTime, httpRqSnap);
