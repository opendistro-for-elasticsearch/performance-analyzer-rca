...
public class ReaderMetricsProcessor implements Runnable {
     ...
     */
    public void parseNodeMetrics(long currTimestamp, boolean testOnly)
            throws Exception {
        long mCurrT = System.currentTimeMillis();
     ...
     * @throws Exception thrown if we have issues parsing metrics
     */
    protected void parseRequestMetrics(String rootLocation, long currWindowStartTime,
     ...
     * @throws Exception thrown if we have issues parsing metrics
     */
    protected void parseHttpRequestMetrics(String rootLocation, long currWindowStartTime,
                                           long currWindowEndTime) throws Exception {
        long mCurrT = System.currentTimeMillis();
    ...
    private void emitShardRequestMetrics(long prevWindowStartTime,
        ...
        if (shardRqMetricsMap.containsKey(prevWindowStartTime)) {
            ...
            LOG.info("shard emit time {}, {}", prevWindowStartTime, preShardRequestMetricsSnapshot.windowStartTime);
    ...
    void parseMasterEventMetrics(String rootLocation, long currWindowStartTime, long currWindowEndTime) {
    ...
    /**
     * OS, Request, Http and master first aligns the currentTimeStamp with a 5 second interval but while
     * looking for the actual file on the disk, they do a further aligning with the 30 second bucket. The
     * node metrics doesn't do the initial 5 second aligning but does a direct 30 second aligning of the
     * given time, before reading files on the disk.
     * OS metrics can do (in some cases) and the Node metrics definitely reads a file from the previous 30
     * second aligned timestamp.
     *
     * Note that previously, the collectors would sample metrics every 5 seconds but write it to a 30 second
     * aligned bucket. So, the files in the current bucket gets overwritten on every write until we hit the
     * 30 second boundary after which a new directory is created and that is used the same way for the next
     * 30 seconds. So any time we are reading from the last bucket, we are reading the last file before the
     * bucket got rotated. So, at the first 5 seconds of the current bucket, if you read the last bucket
     * you are actually reading the file written in last 5 second. If you are reading from the previous bucket
     * in the second 5 second window, then you are actually reading the file written in the last 10 seconds.
     *
     * In the new format, a file (not a directory) is written every 5 seconds. So we can actually read the
     * last 5 second of the data.
    ...
    public void processMetrics(String rootLocation, long currTimestamp) throws Exception {
        ...
        */
        ...
        // Step 2 from above.
        long currWindowStartTime = PerformanceAnalyzerMetrics.getTimeInterval(
                currTimestamp, MetricsConfiguration.SAMPLING_INTERVAL);

        // Step 3 from above.
        currWindowStartTime =
                currWindowStartTime - (2 * MetricsConfiguration.SAMPLING_INTERVAL);
        long currWindowEndTime =
                currWindowStartTime + MetricsConfiguration.SAMPLING_INTERVAL;

        EventProcessor osProcessor =
                OSEventProcessor.buildOSMetricEventsProcessor(currWindowStartTime,
                        currWindowEndTime, conn, osMetricsMap);
        EventProcessor requestProcessor =
                RequestEventProcessor.buildRequestMetricEventsProcessor(currWindowStartTime,
                        currWindowEndTime, conn, shardRqMetricsMap);
        EventProcessor httpProcessor =
                HttpRequestEventProcessor.buildHttpRequestMetricEventsProcessor(currWindowStartTime,
                        currWindowEndTime, conn, httpRqMetricsMap);
        ...
        EventProcessor clusterDetailsEventsProcessor =
                new ClusterDetailsEventProcessor();

        // The event dispatcher dispatches events to each of the registered event processors.
        // In addition to event processing each processor has an initialize/finalize function that is called
        // at the beginning and end of processing respectively.
        // We need to ensure that all the processors are registered, before the initialize function is called.
        ...
        eventDispatcher.initializeProcessing(currWindowStartTime,
                currWindowStartTime + MetricsConfiguration.SAMPLING_INTERVAL);
