...
public class PerformanceAnalyzerApp {
    ...
    private static final ScheduledExecutorService netOperationsExecutor =
            Executors.newScheduledThreadPool(2,
                                             new ThreadFactoryBuilder().setNameFormat("network-thread-%d")
                                                                       .build());
    ...
    public static void main(String[] args) throws Exception {
        ...
        Thread readerThread = new Thread(new Runnable() {
            public void run() {
                while (true) {
                    try {
                        ReaderMetricsProcessor mp = new ReaderMetricsProcessor(settings.getMetricsLocation(), true);
...
//        rcaController.startPollers();
        ...
        boolean useHttps = PluginSettings.instance()
                                         .getHttpsEnabled();
        ...
        RcaController rcaController = new RcaController(netOperationsExecutor, connectionManager, netClient, netServer, httpServer);
        rcaController.startPollers();
    }

    public static HttpServer createInternalServer(PluginSettings settings, int internalPort, NetClient netClient, NetServer netServer) {
        try {
    ...
    private static HttpServer createHttpsServer(int readerPort) throws Exception {
        ...
        if (bindHost != null && !bindHost.trim()
                                         .isEmpty()) {
    ...
    private static int getPortNumber() {
        ...
        try {
            readerPortValue = PluginSettings.instance()
                                            .getSettingValue(WEBSERVICE_PORT_CONF_NAME);

            if (readerPortValue == null) {
                LOG.info("{} not configured; using default value: {}", WEBSERVICE_PORT_CONF_NAME,
    ...
    private static String getBindHost() {
        return PluginSettings.instance()
                             .getSettingValue(WEBSERVICE_BIND_HOST_NAME);
    }
}
