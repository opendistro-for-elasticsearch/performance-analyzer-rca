...
public class FlowUnitWrapper {
    ...
    public static FlowUnitWrapper buildFlowUnitWrapperFromMessage(final FlowUnitMessage value) {
        ...
        if (value.getValuesCount() > 0) {
            flowUnitWrapper.setData(value.getValuesList()
                    .stream()
                    .map(i -> IntStream.range(0, i.getValuesCount())
                            .mapToObj(i::getValues)
                            .collect(Collectors.toList()))
                    .collect(Collectors.toList()));
        }
        if (value.hasResourceContext()) {
            flowUnitWrapper.setResourceContext(ResourceContext.buildResourceContextFromMessage(value.getResourceContext()));
        }
