...
/**
 * This is a serializer - de-serializer class. This is used by the PerformancecAnalyzer Plugin to write to serialize
 * bytes before writing to file and by the PerformanceAnalyzer App to de-serialize the bytes it has read from disk.
 */
public class EventLog {
    ...
    public EventLog() {
        ...
        metricLocPathLength = Paths.get(PluginSettings.instance().getMetricsLocation()).toString().length();
     ...
     *
     * A non-corrupted byte stream should start with the startMarker. It may not end with the endmarker based on how
     * much of the actual bytes on the disk we are reading. This iterates through the bytes and tries to interpret
     * them into Event members. A complete Event object can be something like this:
     *
     * $heap_metrics
     * {"current_time":1566110054768}
     * {"MemType":"totYoungGC","GC_Collection_Event":1, \
     * "GC_Collection_Time":6, "Heap_Committed":-2,"Heap_Init":-2, \
     * "Heap_Max":-2,"Heap_Used":-2}
     ...
     * {"MemType":"Survivor","GC_Collection_Event":-2, \
     * "GC_Collection_Time":-2, "Heap_Committed":11010048, \
     * "Heap_Init":11010048, "Heap_Max":11010048,"Heap_Used":6224432}
     * {"MemType":"OldGen","GC_Collection_Event":-2,"GC_Collection_Time":-2, \
     * "Heap_Committed":179306496,"Heap_Init":179306496, \
     * "Heap_Max":2863661056, "Heap_Used":16384}
     * {"MemType":"Eden","GC_Collection_Event":-2,"GC_Collection_Time":-2, \
     * "Heap_Committed":67108864,"Heap_Init":67108864,"Heap_Max":1409286144, \
     * "Heap_Used":30305704}
     * {"MemType":"NonHeap","GC_Collection_Event":-2,"GC_Collection_Time":-2, \
     * "Heap_Committed":21037056,"Heap_Init":2555904,"Heap_Max":-1, \
     * "Heap_Used":20207152}
     * {"MemType":"Heap","GC_Collection_Event":-2,"GC_Collection_Time":-2, \
     * "Heap_Committed":257425408,"Heap_Init":268435456, \
     * "Heap_Max":3817865216, "Heap_Used":36546520}
     * #
     * $heap_metrics
     * {"current_time":1566110055024}
     * {"MemType":"totYoun
     *
     * A Event object has three members: key, value and epoch.
     * The serializer does not store the epoch explicitly and its part of the
     * data. In the above example:
     * The bytes after '$' in the line, is the key and all the other bytes
     * leading up to '#', but not including it, is the value.
     * Sometimes the last few bytes in themselves cannot make a complete
     * event object, so the return also includes the count of unused
     * bytes. The sender is supposed to send them again, in the next iteration.
     *
     * One corner case is, when the entirety of the bytes cannot create one
     * Event object. This can happen when the value is too large to fit
     * in the buffer. In this case, the de-serialize sends an empty list, The
     * sender then has to read in more bytes and send it again. Sender
    ...
    private static Ret processByte(char b, Ret arg,
