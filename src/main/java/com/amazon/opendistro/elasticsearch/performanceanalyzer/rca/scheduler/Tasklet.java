...
/**
 * This is a task abstraction on top of a Node in the Analysis framework.
 * This is the smallest runnable unit.
 */
public class Tasklet {
     ...
     * @param hopper
     */
    Tasklet(final Node predecessorNode, final Queryable db, final Persistable persistable,
            final Map<Node, List<Node>> remotelyDesirableNodeSet, final WireHopper hopper,
    ...
    void setPredecessorToFutureMap(final Map<Tasklet, CompletableFuture<TaskletResult>> predecessorToFutureMap) {
    ...
    public CompletableFuture<TaskletResult> execute(ExecutorService executorPool) {
        ...
        List<CompletableFuture<TaskletResult>> predecessorResultFutures = predecessors
                .stream()
                .map(p -> predecessorToFutureMap.get(p))
        ...
        CompletableFuture<List<TaskletResult>> futureTasklets = completedPredecessorTasks.thenApplyAsync(
                f ->
                        predecessorResultFutures
                                .stream()
        ...
        CompletableFuture<TaskletResult> retCompletableFuture = futureTasklets.thenApplyAsync(taskletResultsList -> {
            Map<Class, FlowUnit> dependencyDataMap = taskletResultsList
                    .stream()
                    .collect(Collectors.toMap(TaskletResult::getNode, TaskletResult::getFlowUnit));
            LOG.info("RCA: Executing the function for node {}", node.name());
            FlowUnit flowUnit = exec.apply(new OperationArgWrapper(node, db, persistable, hopper, dependencyDataMap));
            Objects.requireNonNull(flowUnit,
                    String.format("ERR: Node '%s' returns a NULL flow unit. If you have nothing to " +
                    "return, try returning FlowUnit.generic() instead.", node.name()));
    ...
    private void sendToRemote(FlowUnit flowUnit) {
        if (remotelyDesirableNodeSet.containsKey(node)) {
            DataMsg dataMsg = new DataMsg(node.name(),
                    remotelyDesirableNodeSet.get(node).stream().map(Node::name).collect(Collectors.toList()),
                    flowUnit);
