...
public class RcaController {
    ...
    public RcaController(final ScheduledExecutorService netOpsExecutorService) {
        ...
        this.useHttps = PluginSettings.instance()
                                      .getHttpsEnabled();
    ...
    private void handleNodeRoleChange(final NodeDetails currentNode, final String electedMasterHostAddress) {
        final NodeRole currentNodeRole = NodeRole.valueOf(currentNode.getRole());
        if (currentNode.getHostAddress()
                       .equalsIgnoreCase(electedMasterHostAddress)) {
    ...
    private void start() {
        ...
        try {
            ...
            WireHopper net = new WireHopper(netPersistor, nodeStateManager, rcaNetClient, subscriptionManager);
            this.rcaScheduler = new RCAScheduler(connectedComponents, db, rcaConf, thresholdMain, persistable, net);
            ...
            rcaScheduler.start();
        } catch (ClassNotFoundException | NoSuchMethodException | InvocationTargetException | InstantiationException | IllegalAccessException | MalformedConfig | SQLException e) {
