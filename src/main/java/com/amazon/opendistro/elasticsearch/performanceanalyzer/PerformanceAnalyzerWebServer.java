...
public class PerformanceAnalyzerWebServer {
  ...
  public HttpServer createInternalServer() {
    final PluginSettings settings = PluginSettings.instance();
    final int internalPort = getPortNumber();
    try {
  ...
  private HttpServer createHttpsServer(int readerPort) throws Exception {
  ...
  private HttpServer createHttpServer(int readerPort) throws Exception {
