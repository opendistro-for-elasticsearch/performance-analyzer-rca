...
/**
 * Class that manages the channel to other hosts in the cluster.
 * It also performs staleness checks, and initiates a new connection if it deems
 * a channel to have gone stale.
 *
 * It also has listens to cluster state changes and manages handling connections to
 * the changed hosts.
 */
public class GRPCConnectionManager {
    ...
    private Map<String, InterNodeRpcServiceGrpc.InterNodeRpcServiceStub> perHostClientStubMap = new HashMap<>();
    ...
    public InterNodeRpcServiceGrpc.InterNodeRpcServiceStub getClientStubForHost(final String remoteHost) {
    ...
    public String getCurrentHostAddress() {
        ...
        // TODO: Maybe fallback on InetAddress.getCurrentHostAddress() method instead of returning empty string.
        return EMPTY_STRING;
    }
    ...
    private ManagedChannel buildInsecureChannel(final String remoteHost) {
        return ManagedChannelBuilder.forAddress(remoteHost, 9600)
                                    .usePlaintext()
                                    .build();
    }
    ...
    private InterNodeRpcServiceGrpc.InterNodeRpcServiceStub buildStubForHost(final String remoteHost) {
        return InterNodeRpcServiceGrpc.newStub(getChannelForHost(remoteHost));
    }

    private void removeAllStubs() {
        for (Map.Entry<String, InterNodeRpcServiceGrpc.InterNodeRpcServiceStub> entry : perHostClientStubMap.entrySet()) {
