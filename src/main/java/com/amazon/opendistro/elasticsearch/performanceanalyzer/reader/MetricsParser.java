...
/**
 * Read metrics files emitted by Elasticsearch in /dev/shm and efficiently load them into tables for
 * further processing.
 */
public class MetricsParser {
  private static final Logger LOG = LogManager.getLogger(MetricsParser.class);

  public static long getThirtySecondBucket(long startTime) {
  ...
  void parseOSMetrics(
      long startTime,
      long endTime,
      OSMetricsSnapshot osMetricsSnap,
      Map<String, List<Event>> metricsDataMap)
      throws Exception {
    List<Event> threadsData = metricsDataMap.get(PerformanceAnalyzerMetrics.sThreadsPath);
    if (threadsData == null) {
      LOG.info("No OS metrics data");
      return;
    }
    BatchBindStep handle = osMetricsSnap.startBatchPut();
    List<String> tidToDelete = new ArrayList<>();
    processOSMetricsForFile(threadsData, osMetricsSnap, startTime, endTime, handle, tidToDelete);
    osMetricsSnap.deleteByTid(tidToDelete);
  ...
  private void processOSMetricsForFile(
      File threadsFile,
      OSMetricsSnapshot osMetricsSnap,
      long startTime,
      long endTime,
      BatchBindStep batchHandle,
      List<String> tidToDelete)
      throws Exception {
  ...
  private void processOSMetricsForFile(
    ...
    boolean retVal = false;
    // threadData gives us all the metric paths starting with "threads/*"
    // LOG.info("OS METRICS {}", threadsData);
    for (Event entry : threadsData) {
      String key = entry.key;
      if (key.contains(PerformanceAnalyzerMetrics.sOSPath)) {
        // ThreadId is embedded in the key as such:
        // threads/14479/os_metrics
  ...
  public void parseRequestMetrics(
      String rootLocation, long startTime, long endTime, ShardRequestMetricsSnapshot rqMetricsSnap)
      throws Exception {
  ...
  public void parseHttpMetrics(
      String rootLocation, long startTime, long endTime, HttpRequestMetricsSnapshot rqMetricsSnap)
      throws Exception {
  ...
  void parseMasterEventMetrics(
    ...
    if (threadsFile.exists()) {
      ...
      try {
        ...
        LOG.error(
            "Failed to parse master metrics with ExceptionCode: "
                + StatExceptionCode.READER_PARSER_ERROR.toString(),
            e);
        StatsCollector.instance().logException(StatExceptionCode.READER_PARSER_ERROR);
      }
  ...
  private void emitMasterStartFinishMetrics(
      long startTime,
      long endTime,
  ...
  private void emitStartMasterEventMetric(
      String startMetrics, String insertOrder, String threadId, BatchBindStep handle) {

    String priority =
        PerformanceAnalyzerMetrics.extractMetricValue(
            startMetrics, MasterMetricDimensions.MASTER_TASK_PRIORITY.toString());

    long st =
        Long.parseLong(
            PerformanceAnalyzerMetrics.extractMetricValue(
                startMetrics, CommonMetric.START_TIME.toString()));

    String taskType =
        PerformanceAnalyzerMetrics.extractMetricValue(
            startMetrics, MasterMetricDimensions.MASTER_TASK_TYPE.toString());

    String taskMetadata =
        PerformanceAnalyzerMetrics.extractMetricValue(
            startMetrics, MasterMetricDimensions.MASTER_TASK_METADATA.toString());

    long queueTime =
        Long.parseLong(
            PerformanceAnalyzerMetrics.extractMetricValue(
                startMetrics, MasterMetricDimensions.MASTER_TASK_QUEUE_TIME.toString()));

    handle.bind(threadId, insertOrder, priority, taskType, taskMetadata, queueTime, st, null);
  }

  private void emitEndMasterEventMetric(
      String startMetrics, String insertOrder, String threadId, BatchBindStep handle) {

    long finishTime =
        Long.parseLong(
            PerformanceAnalyzerMetrics.extractMetricValue(
                startMetrics, CommonMetric.FINISH_TIME.toString()));
  ...
  private void emitStartHttpMetric(
    ...
    String startTimeVal =
        PerformanceAnalyzerMetrics.extractMetricValue(
            startMetrics, HttpMetric.START_TIME.toString());
    String itemCountVal =
        PerformanceAnalyzerMetrics.extractMetricValue(
            startMetrics, HttpMetric.HTTP_REQUEST_DOCS.toString());
    try {
      long st = Long.parseLong(startTimeVal);
      String indices =
          PerformanceAnalyzerMetrics.extractMetricValue(
              startMetrics, HttpDimension.INDICES.toString());
  ...
  private void emitFinishHttpMetric(
      File metricFile, String rid, String operation, BatchBindStep handle) {
    String finishMetrics = PerformanceAnalyzerMetrics.getMetric(metricFile.getAbsolutePath());

    String finishTimeVal =
        PerformanceAnalyzerMetrics.extractMetricValue(
            finishMetrics, HttpMetric.FINISH_TIME.toString());
    String status =
        PerformanceAnalyzerMetrics.extractMetricValue(
            finishMetrics, HttpDimension.HTTP_RESP_CODE.toString());
    String exception =
        PerformanceAnalyzerMetrics.extractMetricValue(
            finishMetrics, HttpDimension.EXCEPTION.toString());
    try {
  ...
  private boolean processOSMetrics(
    ...
    if (opFileLastModified < startTime) {
      LOG.info("File last modified time is less than startTime - {}", opFile.getAbsolutePath());
      return false;
    }
    ...
    if (lastUpdateTimePerTid.containsKey(threadID)) {
      if (lastUpdateTimePerTid.get(threadID) == opFileLastModified) {
        LOG.info(
            "File last modified time is equal to the last updated time - {}",
            lastUpdateTimePerTid.get(threadID));
        return false;
      }
    ...
    for (OSMetrics metric : metrics) {
      try {
        String metricVal =
            PerformanceAnalyzerMetrics.extractMetricValue(sOSMetrics, metric.toString());
        if (metricVal != null) {
    ...
    String threadName =
        PerformanceAnalyzerMetrics.extractMetricValue(
            sOSMetrics, OSMetricsCollector.MetaDataFields.threadName.toString());
  ...
  private boolean processOSMetrics(
    ...
    Map<String, String> processedData = extrackKeyValFromData(osMetricsdata);
  ...
  private void emitStartMetric(
      String startMetrics, String rid, String threadId, String operation, BatchBindStep handle) {
    long st =
        Long.parseLong(
            PerformanceAnalyzerMetrics.extractMetricValue(
                startMetrics, ShardBulkMetric.START_TIME.toString()));
    String indexName =
        PerformanceAnalyzerMetrics.extractMetricValue(
            startMetrics, ShardBulkDimension.INDEX_NAME.toString());
    String shardId =
        PerformanceAnalyzerMetrics.extractMetricValue(
            startMetrics, ShardBulkDimension.SHARD_ID.toString());
    String primary =
        getPrimary(
            PerformanceAnalyzerMetrics.extractMetricValue(
                startMetrics, ShardBulkDimension.PRIMARY.toString()));
    String docCountString =
        PerformanceAnalyzerMetrics.extractMetricValue(
            startMetrics, ShardBulkMetric.ITEM_COUNT.toString());
    long docCount = 0;
  ...
  private void emitFinishMetric(
      String finishMetrics, String rid, String threadId, String operation, BatchBindStep handle) {
    long ft =
        Long.parseLong(
            PerformanceAnalyzerMetrics.extractMetricValue(
                finishMetrics, ShardBulkMetric.FINISH_TIME.toString()));
    String indexName =
        PerformanceAnalyzerMetrics.extractMetricValue(
            finishMetrics, ShardBulkDimension.INDEX_NAME.toString());
    String shardId =
        PerformanceAnalyzerMetrics.extractMetricValue(
            finishMetrics, ShardBulkDimension.SHARD_ID.toString());
    String primary =
        getPrimary(
            PerformanceAnalyzerMetrics.extractMetricValue(
                finishMetrics, ShardBulkDimension.PRIMARY.toString()));
    handle.bind(shardId, indexName, rid, threadId, operation, primary, null, ft, null);
  }

  private void handleidFile(
      File idFile,
      String threadID,
      long startTime,
      long endTime,
