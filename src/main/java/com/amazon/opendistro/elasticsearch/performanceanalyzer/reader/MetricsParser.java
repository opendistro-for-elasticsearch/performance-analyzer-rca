...
public class MetricsParser {
    ...
    void parseOSMetrics(long startTime, long endTime,
                        OSMetricsSnapshot osMetricsSnap, Map<String, List<Event>> metricsDataMap) throws Exception {
    ...
    private void processOSMetricsForFile(File threadsFile,
    ...
    private void processOSMetricsForFile(List<Event> threadsData,
                                         OSMetricsSnapshot osMetricsSnap, long startTime, long endTime,
                                         BatchBindStep batchHandle, List<String> tidToDelete) throws Exception {
        ...
        for (Event entry: threadsData) {
            ...
            if (key.contains(PerformanceAnalyzerMetrics.sOSPath)) {
                ...
                retVal = processOSMetrics(entry.value, threadID, lastUpdateTimePerTid, startTime,
                        endTime, batchHandle, tidToDelete)
                        || retVal;
    ...
    void parseMasterEventMetrics(String rootLocation,
                                 long startTime,
                                 long endTime,
    ...
    private boolean processOSMetrics(File opFile, String threadID,
        ...
        if (lastUpdateTimePerTid.containsKey(threadID)) {
            if (lastUpdateTimePerTid.get(threadID) == opFileLastModified) {
                LOG.info("File last modified time is equal to the last updated time - {}",
    ...
    // TODO: As everything is now dumped in 1 single file per 5 seconds. We can check the last modified time
    ...
    private boolean processOSMetrics(String osMetricsdata, String threadID,
                                     Map<String, Long> lastUpdateTimePerTid,
                                     long startTime, long endTime,
                                     BatchBindStep batchHandle, List<String> tidToDelete) throws Exception {
        Map<String, Double> osMetrics = new HashMap<>();
        ...
        if (opFileLastModified > endTime) {
            opFileLastModified = endTime;
            LOG.info("File last modified {} time is greater than endTime - {}", opFileLastModified, endTime);
        }
        //Discard os metrics if the file has not been updated in the 5 second window.
        if (opFileLastModified < startTime) {
            return false;
    ...
    /**
     * An example value is this:
     * current_time:1566413987194
     * StartTime:1566413987194
     * ItemCount:359
     * IndexName:nyc_taxis
     * ShardID:25
     * Primary:true
     * Each pair is separated by new line and the key and value within each pair is separated by ":"
     * This function just parses the string and generates the map as such.
