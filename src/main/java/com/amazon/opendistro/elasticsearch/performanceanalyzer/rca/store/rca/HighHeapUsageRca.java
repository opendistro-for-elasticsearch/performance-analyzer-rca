 ...
 * This RCA subscribes three metrics :the current old gen usage(Heap_Used), the maximum heap size allowed(Heap_Max) and
 * the full GC count during the last time interval(Gc_Event) and the RCA keeps pushing the heap usage and GC count into
 * the sliding window and keeps track of the max heap.
 * When a full GC occurs, this RCA tries to capture the minimum old gen usage right after this full GC and those are
 * the long-lived objects we are interested.The sliding window can check both the sum of gc event and the min heap usage
 * in O(1) time complexity. To check whether the old gen is healthy, we first check the sum of gc event. if it is a
 * non zero value, it means there is at least one full GC during the entire sliding window and then compare the usage
 * with the threshold. To git rid of false positive from sampling, we keep the sliding window
 * big enough to keep at least a couple of such minimum samples to make the min value more accurate.
 */
...
public class HighHeapUsageRca extends Rca {
    ...
    //FullGC needs to occur at least once during the entire sliding window in order to capture the minimum
    private static final double OLD_GEN_GC_THRESHOLD = 1;
    private static final double CONVERT_BYTES_TO_MEGABYTES = Math.pow(1024, 3);

    public <M extends Metric> HighHeapUsageRca(long evaluationIntervalSeconds, final M heap_Used, final M gc_event, final M heap_Max) {
    ...
    public ResourceContext determineHeapUsageState() {
        ...
        if (samplingDataSlidingWindow.getOldGGenGCEvent() >= OLD_GEN_GC_THRESHOLD
                && !Double.isNaN(currentMinOldGenUsage) && currentMinOldGenUsage/maxOldGenHeapSize > OLD_GEN_USED_THRESHOLD_IN_PERCENTAGE) {
            LOG.debug("heapUsage is above threshold. OldGGenGCEvent = {}, oldGenUsage percentage = {}", samplingDataSlidingWindow.getOldGGenGCEvent(), currentMinOldGenUsage/maxOldGenHeapSize);
    ...
    public ResourceFlowUnit operate() {
        ...
        for (MetricFlowUnit heapUsedMetric : heapUsedMetrics) {
            ...
            double ret = heapUsedMetric.getDataFromMetric(MEM_TYPE.toString(), OLD_GEN.toString(), MetricsDB.MAX);
            if (Double.isNaN(ret)) {
        ...
        for (MetricFlowUnit gcEventMetric : gcEventMetrics) {
            if(gcEventMetric.isEmpty()) { continue; }
             double ret = gcEventMetric.getDataFromMetric(MEM_TYPE.toString(), TOT_FULL_GC.toString(), MetricsDB.MAX);
             if (Double.isNaN(ret)) {
        ...
        for (MetricFlowUnit heapMaxMetric : heapMaxMetrics) {
            ...
            double ret = heapMaxMetric.getDataFromMetric(MEM_TYPE.toString(), OLD_GEN.toString(), MetricsDB.MAX);
            if (Double.isNaN(ret)) {
        ...
        if (!Double.isNaN(oldGenHeapUsed)) {
            LOG.debug("oldGenHeapUsed = {}, oldGenGCEvent = {}, maxOldGenHeapSize = {}", oldGenHeapUsed, oldGenGCEvent, maxOldGenHeapSize);
            samplingDataSlidingWindow.add(System.currentTimeMillis(), oldGenHeapUsed, oldGenGCEvent);
        }

        if (counter == RCA_PERIOD) {
            List<List<String>> ret = new ArrayList<>();
            ret.addAll(Arrays.asList(Collections.singletonList("Node ID"), Collections.singletonList(RcaUtil.fetchCurrentNodeId())));
    ...
    private static class SamplingDataSlidingWindow {
        ...
        public void add(long timeStamp, double oldGenUsage, int oldGenGCEvent) {
            ...
            while (!oldGGenGCEventDeque.isEmpty()
                    && TimeUnit.MILLISECONDS.toSeconds(timeStamp - oldGGenGCEventDeque.peekLast().getKey()) > SLIDING_WINDOW_SIZE_IN_MINS*60) {
            ...
            while (!oldGenUsageDeque.isEmpty()
                    && TimeUnit.MILLISECONDS.toSeconds(timeStamp - oldGenUsageDeque.peekLast().getKey()) > SLIDING_WINDOW_SIZE_IN_MINS*60) {
                oldGenUsageDeque.pollLast();
            }
        }
        public int getOldGGenGCEvent() { return oldGGenGCEventDeque.size(); }
