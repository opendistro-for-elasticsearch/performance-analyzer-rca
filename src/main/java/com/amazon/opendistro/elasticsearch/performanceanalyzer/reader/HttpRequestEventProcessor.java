...
public class HttpRequestEventProcessor implements EventProcessor {
    ...
    static HttpRequestEventProcessor buildHttpRequestMetricEventsProcessor(long currWindowStartTime,
                                                                           long currWindowEndTime,
                                                                           Connection conn,
                                                                           NavigableMap<Long,
                                                                     HttpRequestMetricsSnapshot> httpRqMetricsMap)
            throws Exception {
        if (httpRqMetricsMap.get(currWindowStartTime) == null) {
            ...
            Map.Entry<Long, HttpRequestMetricsSnapshot> entry =
                    httpRqMetricsMap.lastEntry();
            if (entry != null) {
    ...
    private void emitStartHttpMetric(Event entry, String[] keyItems) {
        ...
        try {
            ...
            LOG.error("Unable to parse string. StartTime:{}, itemCount:{}, ExcepionCode: {},\n startMetrics:{}",
                    startTimeVal, itemCountVal, StatExceptionCode.READER_PARSER_ERROR.toString(), entry.key);
    ...
    private void emitFinishHttpMetric(Event entry, String[] keyItems) {
        ...
        try {
            ...
            LOG.error("Unable to parse string. FinishTime:{} ExcepionCode: {} \n finishMetrics:{}",
                    finishTimeVal, StatExceptionCode.READER_PARSER_ERROR.toString(), entry.key);
            StatsCollector.instance().logException(StatExceptionCode.READER_PARSER_ERROR);
            throw e;
        }
    }
