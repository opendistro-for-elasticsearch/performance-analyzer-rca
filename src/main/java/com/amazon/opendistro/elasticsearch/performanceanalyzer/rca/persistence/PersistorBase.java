...
public abstract class PersistorBase implements Persistable {
    ...
    PersistorBase(String dir, String filename, String dbProtocolString) throws SQLException {
        ...
        this.filename = String.format("%s.%s", Paths.get(dir, filename).toString(), dateFormat.format(this.fileCreateTime));
    ...
    public synchronized String read() {
        // Currently this method only contains a call to readTables() - later could add the part to read multiple sqlite files
    ...
    public synchronized void openNewDBFile() throws SQLException {
        this.fileCreateTime = new Date(System.currentTimeMillis());
        this.filename = String.format("%s.%s", Paths.get(dir, filenameParam).toString(), dateFormat.format(this.fileCreateTime));
        this.tableNames = new HashSet<>();
    ...
    public synchronized String getFilesInDirDB(String datePrefix) {
        ...
        File[] files = dirDB.listFiles(new FilenameFilter() {
            @Override
    ...
    public synchronized String getDBFilePath(int hours) throws ParseException {
        Date hoursBeforeFileCreateMs = new Date(this.fileCreateTime.getTime() - ((long)hours * 60 * 60 * 1000));
    ...
    public synchronized void deleteOldDBFile() throws SQLException, SecurityException, ParseException {
    ...
    // The database is always rotated when the thread dies. So the tablenames in the tableNames Set is the
    ...
    public synchronized void write(Node node, ResourceFlowUnit flowUnit) {
        ...
        try {
            ...
            LOG.error("RCA: Caught SQLException while creating a new DB connection for file rotation.", e);
        }
