...
public class NetServer extends InterNodeRpcServiceGrpc.InterNodeRpcServiceImplBase implements Runnable {
    ...
    @Override
    public void run() {
        LOG.info("Starting the gRPC server on port {} with {} threads. Using HTTPS: {}", port, numServerThreads, useHttps);
    ...
    private Server buildHttpsServer() {
        return NettyServerBuilder.forPort(port)
            .addService(this)
            .workerEventLoopGroup(new NioEventLoopGroup(numServerThreads))
            .useTransportSecurity(CertificateUtils.getCertificateFile(), CertificateUtils.getPrivateKeyFile())
            .build();
    }
     ...
     */
    @Override
    public StreamObserver<FlowUnitMessage> publish(final StreamObserver<PublishResponse> responseObserver) {
     ...
     */
    @Override
    public void subscribe(final SubscribeMessage request, final StreamObserver<SubscribeResponse> responseObserver) {
